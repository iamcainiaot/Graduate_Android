...
import lib_utils.CipherUtil;
import lib_utils.FileUtil;
import lib_utils.MyLogUtil;

/**
 * @author taozhu5
 * @date 2019/3/13 16:42
 * @description 数据磁盘缓存管理类(缓存统一放置到 / sdcard / Android / data / { 包名 }
 *files / data_cache / 文件夹下)
 */
...
public class DiskCacheManager {
    ...
    private DiskCacheManager(Context context) {
        ...
        DIR_CACHE_ROOT = FileUtil.getExternalFilesDir(context, DATA_CACHE_DIR_NAME);
    }
    ...
    public <T> T getCacheData(Class<T> returnType, String filePrefix) {
        ...
        try {
            ...
            MyLogUtil.e("get savedVersion failed " + cacheFileRule, e);
        }
        ...
        if (!TextUtils.isEmpty(TDataStr)) {
            try {
                ...
                MyLogUtil.e("JsonSyntaxException ", e);
            }
        }
        return null;
    }
